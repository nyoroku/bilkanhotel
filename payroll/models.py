...
class PayrollEntry(models.Model):
    ...
    reward_points_redeemed = models.PositiveIntegerField(default=0,
                                                         help_text="Number of waiter reward points redeemed as cash in this payroll")
    reward_points_value = models.DecimalField(max_digits=10, decimal_places=2, default=0.00,
    ...
    def calculate_financials(self, commit=True):
        ...
        one_time_allowances = self.adjustments.filter(type='ALLOWANCE').aggregate(total=Sum('amount'))[
                                  'total'] or Decimal('0.00')
        one_time_deductions = self.adjustments.filter(type='DEDUCTION').aggregate(total=Sum('amount'))[
                                  'total'] or Decimal('0.00')

        # 2. Get all recurring allowances/deductions for the user
        recurring_allowances = self.user.recurring_adjustments.filter(type='ALLOWANCE').aggregate(total=Sum('amount'))[
                                   'total'] or Decimal('0.00')
        recurring_deductions = self.user.recurring_adjustments.filter(type='DEDUCTION').aggregate(total=Sum('amount'))[
                                   'total'] or Decimal('0.00')

        # 3. NEW: Get reward points value as an allowance
        reward_allowance = self.reward_points_value

        self.base_pay = self.user.basic_salary
        self.total_allowances = one_time_allowances + recurring_allowances + reward_allowance
