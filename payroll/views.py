...
from pos.models import WaiterRewardSettings
...
def run_new_payroll_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            active_users = User.objects.filter(is_active=True, basic_salary__gt=0)
            if not active_users.exists():
                messages.warning(request, "No active employees with a set salary were found.")
